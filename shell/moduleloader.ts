...
// The local launcher ships the broadly compatible Asyncify binaries. JSPI is
// opt-in until matching binaries are explicitly included in a release.
function useJSPI(): boolean {
    const param = urlParams.get('jspi');
    return param === '1' && typeof (WebAssembly as any).Suspending === 'function';
}
